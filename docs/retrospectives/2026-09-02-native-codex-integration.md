...
The final native-history review found that the retained host identity was not yet used by timeline reads after environment pruning, and that the conversation outline still projected only local events. Native timeline and outline reads now share one provider-history projection: routing uses the retained native host, provider metadata supplies the workspace root when the environment is gone, and the complete outline reuses the same stable row IDs as paginated timeline pages. The outline must project full turn detail before reducing to its own compact items; asking the timeline projector for summary detail can legitimately omit an opening user message when the assistant response is the turn's terminal message.
