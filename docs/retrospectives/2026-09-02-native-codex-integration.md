...
The exact-head archive review found four final consistency hazards. A local unarchive could race the awaited provider archive, adoption could race the same native session through a different entry point, a general source-derived creation path could bypass the dedicated fork lock, and hidden forks were validated only after their source had already changed state. Spaceship now serializes adoption and both archive entry points by host, provider, and native session ID; serializes local archive, unarchive, and every source-derived creation path by projected source thread; preflights every hidden fork before contacting the provider; and keeps the durable provider confirmation compatible across the migration's earlier published hash. The confirmation suppresses automatic settlement and restart duplication once it is recorded, while a new explicit archive request still reaches the provider because the native session may have been unarchived elsewhere. Durable recovery for a crash between provider success and that local write remains a follow-up. The associated tests also had to model native history faithfully: turn-scoped events require a turn start, and conversation rows inside a native turn must be inspected recursively rather than treated as top-level rows.
