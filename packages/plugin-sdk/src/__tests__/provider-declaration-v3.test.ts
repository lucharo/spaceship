...
describe("provider declaration fields renamed in SDK 0.4.16", () => {
  ...
  it("rejects a malformed native session history capability", () => {
    const base = declaration();
    const malformed = {
      ...base,
      capabilities: {
        ...base.capabilities,
        experimental_supportsNativeSessionHistory: "yes",
      },
    };
    expect(() => validatePluginProviderDeclaration(malformed as never)).toThrow(
      /capabilities\.experimental_supportsNativeSessionHistory must be a boolean/u,
    );
  });
});
